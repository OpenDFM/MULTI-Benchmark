"""DeepSeek-Janus evaluator with HuggingFace Transformers"""
import pdb
import torch
...
class DeepSeekJanusEvaluator:
    def __init__(self, model_dir="deepseek-ai/Janus-Pro-7B", max_tokens=2000, device_map="auto", use_flash_attention=True):
        ...
        vl_chat_processor: VLChatProcessor = VLChatProcessor.from_pretrained(self.model_dir)
        self.processor = vl_chat_processor
        self.tokenizer = vl_chat_processor.tokenizer
        
        vl_gpt: MultiModalityCausalLM = AutoModelForCausalLM.from_pretrained(self.model_dir, trust_remote_code=True)
    ...
    def prepare_inputs(self, question):
        ...
        if len(image_list) == 1:
            ...
            image_content = ''.join([f'This is image_{index}: <image_placeholder>\n' for index in range(len(image_list))])
            inputs = [{
                "role": "<|User|>",
    ...
    def generate_response(self, input):
        if isinstance(input, dict):
            ...
            outputs = self.model.language_model.generate(inputs_embeds=inputs_embeds, attention_mask=prepare_inputs.attention_mask, pad_token_id=self.tokenizer.eos_token_id, bos_token_id=self.tokenizer.bos_token_id, eos_token_id=self.tokenizer.eos_token_id, max_new_tokens=512, do_sample=False, use_cache=True)
