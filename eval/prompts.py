...
answer_extractor_prompt="""我将会给你一名学生关于一个问题的解答过程，请帮我从中提取出这名学生给出的答案。你需要根据学生的解答过程，判断题目的类型，并根据下面的对比输出相应格式的答案。
- 单选：这道题目只有唯一的正确选项，请只给出唯一一个大写英文字母作为答案，不包含选项后面的描述，如：A，B，E。
- 多选：这道题目有不小于两个可行的答案，请选出所有的正确选项，格式为连续的多个大写英文字母，不包含选项后面的描述，如：AC，BDE。
- 填空：每一个 '[MASK]' 对应一个最简且确定的答案，多个 '[MASK]' 的答案之间换行隔开，如：文艺复兴\n0.5\n①。
- 解答：这道题需要对问题进行分析作答，请覆盖所有要点的基础上尽可能简练的表达，请以'我的分析如下：'作为开头。
你不需要修饰、改写、润色学生的解答，也不要输出诸如'提取的答案是...'或'好的我可以帮助你...'这类无关的内容，只需要根据下面的解答提取出学生的答案即可，如果解答过程中无法提取出答案，请回复'没有给出答案'。"""
